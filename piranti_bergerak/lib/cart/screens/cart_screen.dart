...
class _CartScreenContent extends StatelessWidget {
            ...
            child: RefreshIndicator(
              ...
              child: ScrollConfiguration(
                ...
                child: ListView.builder(
                  physics: const AlwaysScrollableScrollPhysics(parent: SlowDownScrollPhysics(velocityFactor: 0.5, dragFactor: 0.65)),
                  padding: const EdgeInsets.only(top: 12, bottom: 8),
                  itemCount: provider.items.length + 2, // items + promo + summary
  ...
  Future<void> _confirmDelete(BuildContext context, CartProvider provider, String itemId) async {
    ...
    final confirmed = await showDialog<bool>(
          ...
          builder: (innerContext) {
            return AlertDialog(
              ...
              actions: [
                TextButton(onPressed: () => Navigator.of(innerContext).pop(false), child: const Text('Batal')),
                FilledButton(onPressed: () => Navigator.of(innerContext).pop(true), child: const Text('Hapus')),
              ],
            );
          },
        ) ??
    ...
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Produk berhasil dihapus dari keranjang.')));
  }
}
